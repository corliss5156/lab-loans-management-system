...
function ResetpasswordStudent() {
  ...
  const onSubmit = (data, e) => {
    axios.put(API_HOST + "/student/changepassword", {
      email: email, 
      password: password
    }).then((response)=>{
      setSuccess(true)
        ...
        <Form onSubmit = {handleSubmit(onSubmit, onError)}> 
          ...
          <Form.Group> 
            ...
            validate:{
              ...
              }})} onChange = {(e)=>setCpassword(e.target.value)} className = "height" type = "password" placeholder = "Confirm password"/>
            {errors.cPassword && <Form.Text className  = "error"> {errors.cPassword.message}</Form.Text>}
          </Form.Group> 
          {success &&<Form.Text>Password has been successfully reset. <Link to = '/student/home'> Back to home</Link> </Form.Text>}
